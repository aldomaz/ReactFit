...
function NormalRoutine(props ) {
    ...
    const finishRoutineAlert = () => {
        Alert.alert("Finalizar asignación de rutina", "¿Estás Seguro?", [
            {text: 'Sí', onPress: () => props.navigation.navigate('NormalList')},
    ...
    const cancelRoutine = async () => {
        ...
        props.navigation.navigate('NormalList');
    }
    ...
    useEffect (() => {
        ...
        if(user.username === ''){
            setUser({
                username: props.route.params.userId,
            })
        }  
    }, [])
